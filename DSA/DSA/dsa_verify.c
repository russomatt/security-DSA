...
int _dsa_verify_hash (mp_int *r, mp_int *s, mp_int *hash,mp_int *keyG, mp_int *keyP, mp_int *keyQ, mp_int *keyY)
{
	mp_int w, v, u1, u2;
	int ret;
...
int dsa_verify_sign(const char *data, int dataLen, const unsigned char* keyData, const char* sigR, const char* sigS)
{
	// dss1 hashing algorithm is actually sha256
	Sha256Context ctx;
	uint8_t digest[SHA256HashSize];

	 sha256_init_hash(Sha256Context *context);
	 sha256Update(Sha256Context *context, const void *data, size_t length);
	 sha256_final_hash(Sha256Context *context, uint8_t *digest);

	mp_int hash, keyG, keyP, keyQ, keyY, r, s;
	mp_init_multi(&hash, &keyG, &keyP, &keyQ, &keyY, &r, &s, NULL);
	mp_read_unsigned_bin(&hash, digest, sizeof(digest));
...
int main()
{
	char *message = "This is security project"; //hard-coded message for validation of sign for testing purpose
	...
	return dsa_verify_sign(
		message, strlen(message),
		publicKey,
		"D795D68F0CFB19F8A5C042B6427DB8132D1403D1",
		"5D1E9010B9B0605BA6F0983CF49A14FD6F18892D"); // hard-coded values for validation of sign for testing purpose
}
